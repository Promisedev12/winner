...
export default function SettingsPage() {
      ...
      if (response.success) {
        setSettings({
          ...
          auto_approve_reviews:
            response.data.auto_approve_reviews === 1 ||
            response.data.auto_approve_reviews === true,
          default_price: parseFloat(response.data.default_price) || 0,
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const response = await authorService.updateSettings({
        notification_email: settings.notification_email ? 1 : 0,
        auto_approve_reviews: settings.auto_approve_reviews ? 1 : 0,
    ...
    <div className='max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
      <div className='flex justify-between items-center mb-8'>
        <div>
          ...
          <p className='text-slate-400 mt-1'>Manage your author preferences</p>
        </div>
        <button
      ...
      <div className='space-y-6'>
            ...
            <label className='flex items-center justify-between cursor-pointer'>
              <div>
                ...
                <p className='text-xs text-slate-500'>
                  Receive email when someone reviews your books
                </p>
              </div>
              <button
                type='button'
        ...
        <div className='bg-slate-800 rounded-xl p-6 border border-slate-700'>
          <h3 className='text-lg font-semibold text-white mb-4 flex items-center gap-2'>
            <FiMessageCircle /> Review Settings
          </h3>
          <div className='space-y-4'>
            <label className='flex items-center justify-between cursor-pointer'>
              <div>
                <span className='text-slate-300'>Auto-approve Reviews</span>
                <p className='text-xs text-slate-500'>
                  Automatically approve reviews without moderation
                </p>
              </div>
              <button
                type='button'
                ...
                className={`relative w-12 h-6 rounded-full transition-colors ${settings.auto_approve_reviews ? 'bg-emerald' : 'bg-slate-600'}`}
              >
                <div
                  className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${settings.auto_approve_reviews ? 'right-1' : 'left-1'}`}
                ></div>
              </button>
            </label>
          </div>
